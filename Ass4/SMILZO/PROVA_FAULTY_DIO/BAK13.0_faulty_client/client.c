...
int main(int argc, char **argv) {
  ...
  {
    int bytesread, len = BUFSIZE, try = 0;
    unsigned int expected_chunk_no = 1, wrong_packets = 0, no_response = 0, x;
    ...
    while (len >= BUFSIZE) {
      ...
      if (err == 0) {
        ...
        if ((audio_chunk.msg_counter % 2000 == 0) &&  try < 5 ) {
          x = audio_chunk.msg_counter + 1;
          err = sendto(server_fd, &x, sizeof(audio_chunk.msg_counter),
