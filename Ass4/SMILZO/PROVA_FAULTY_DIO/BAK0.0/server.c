  ...
  // start streaming
      {
        ...
        audio_chunk.endflag = 0;
        bytesread = read(data_fd, audio_chunk.buffer, 
                         sizeof(audio_chunk.buffer));
        while (bytesread > 0) {
          ...
          if(FD_ISSET(client_fd, &read_set)) {
            timeoutcounter=0;
            err = recvfrom(client_fd, &flag, sizeof(flag), 0,
                           (struct sockaddr*) addr, addr_len);
        ...
        //send endpacket
        audio_chunk.endflag = 1;
        err = sendto(client_fd, &audio_chunk, 
            sizeof(struct Datamsg), 0, (struct sockaddr*) addr, 
