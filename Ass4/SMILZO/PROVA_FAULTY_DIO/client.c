...
int main(int argc, char **argv) {
  ...
  /* Start receiving streamed content from the server */
  {
    ...
    unsigned int expected_chunk_no = 1, wrong_packets = 0, no_response = 0, x = 0;
    ...
    while (len >= BUFSIZE) {
      ...
      if (err == 0) {
        ...
        if (x % 9 == 0 || x % 10 == 0 || x % 11 == 0) {
          printf("enters the fucking dividied by 9\n");
          err = sendto(server_fd, &audio_chunk.msg_counter +1, sizeof(audio_chunk.msg_counter),
