...
int resend_lost_packet(int rsends, fd_set *set, int fd, struct timeval *t) {
  printf("Inside recursive method\nThis is rsends: %d\n", rsends);
  if (rsends > RESEND_PACKET_LIMIT) {
    return CLIENT_NOT_RESPONDING;
  }
  int err = wait_for_response(set, fd, t);
  error_handling(err, "Error while monitoring file descriptors");
  if (err == 0) {
    resend_lost_packet(++rsends, set, fd, t);
  }
  return 0;
}
...
int resend_lost_packet2(fd_set *set, int fd, struct timeval *t) {
  int i, err;
  ...
  // start streaming
      {
        ...
        while ((bytesread = read(data_fd, audio_chunk.buffer, sizeof(audio_chunk.buffer))) > 0) {
          ...
          err = sendto(client_fd, &audio_chunk, sizeof(struct Datamsg), 0,
                       (struct sockaddr*) addr, sizeof(struct sockaddr_in));
          printf("Before resend lost packet call in if\n");
          err = resend_lost_packet(0, &read_set, client_fd, &timeout);
          flag = -1;
          printf("This is the flag di porco: %d\n", flag_calc());
          printf("This is the error of resend lost dio porco: %d", err);
          if (err < 0) {
